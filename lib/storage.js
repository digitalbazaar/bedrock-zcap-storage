...
authorizations.get = async ({id, controller, invocationTarget} = {}) => {
  ...
  const record = await collection.findOne(
    query, {projection: {_id: 0, authorization: 1, meta: 1}});
  if(!record) {
    throw new BedrockError(
 ...
 */
authorizations.find = async ({query = {}, options = {}}) => {
  const collection = database.collections['zcap-storage-authorization'];
...
zcaps.get = async ({controller, id, referenceId} = {}) => {
  ...
  const collection = database.collections['zcap-storage-zcap'];
  const record = await collection.findOne(
    query, {projection: {_id: 0, capability: 1, meta: 1}});
  if(!record) {
    throw new BedrockError(
      'Authorization capability not found.',
      'NotFoundError',
 ...
 * @return {Promise<Array>} resolves to the records that matched the query.
 */
zcaps.find = async ({query = {}, options = {}}) => {
  const collection = database.collections['zcap-storage-zcap'];
  return collection.find(query, options).toArray();
};
