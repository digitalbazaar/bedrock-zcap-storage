 ...
 *   authorization.
 * @param {string} [invocationTarget] the `invocationTarget` for the capability.
 *
 ...
 *   `false` if not.
 */
zcaps.remove = async ({controller, id}) => {
  assert.string(controller, 'controller');
  assert.string(id, 'id');

  const result = await database.collections.zcap.remove(
    {invoker: database.hash(controller), id: database.hash(id)});
  return result.result.n !== 0;
};
