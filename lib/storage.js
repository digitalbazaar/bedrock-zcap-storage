...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  */
  ...
  await promisify(database.createIndexes)([{
    ...
    // cover capability queries by storer and id; allows for delegators
    // to see all of the zcaps they have delegated
    collection: 'delegatedZCap',
    fields: {storer: 1, id: 1},
    options: {unique: true, background: false}
  }, {
 ...
 *
 * @param {string} storer the ID of the entity that is storing the
 *   capability.
 * @param {Object} capability the capability to insert.
 *
 * @return {Promise<Object>} resolves to the database record.
 */
delegated.insert = async ({storer, capability}) => {
  assert.string(storer, 'storer');
 ...
 * capability, either `storer` or `invocationTarget` must be given.
 *
 * @param {string} id the ID of the capability.
 * @param {string} [storer] the ID of the entity that stored the zcap.
 * @param {string} [invocationTarget] the `invocationTarget` for the zcap.
 *
 * @return {Promise<Object>} resolves to `{capability, meta}`.
 */
delegated.get = async ({id, storer, invocationTarget}) => {
  assert.string(id, 'id');
  assert.optionalString(storer, 'storer');
  assert.optionalString(invocationTarget, 'invocationTarget');
  if(!(storer || invocationTarget)) {
    throw new TypeError(
      'Either "storer" or "invocationTarget" must be given.');
  }
  ...
  if(storer) {
    query.storer = database.hash(storer);
  }
  if(invocationTarget) {
  ...
  if(!record) {
    throw new BedrockError(
      ...
      'NotFoundError',
      {id, invocationTarget, storer, httpStatusCode: 404, public: true});
  }
 ...
 *   `false` if not.
 */
delegated.remove = async ({storer, id}) => {
  assert.string(storer, 'storer');
  assert.string(id, 'id');

  const result = await database.collections.delegatedZCap.remove(
    {storer: database.hash(storer), id: database.hash(id)});
