...
const logger = bedrock.loggers.get('app').child('bedrock-zcap-storage');
...
authorizations.find = async ({query = {}, fields = {}, options = {}}) => {
  ...
  if(fields) {
    logger.info('The parameter fields in method authorizations.find is being ' +
...
zcaps.find = async ({query = {}, fields = {}, options = {}}) => {
  ...
  // instead use options.projection
  if(fields) {
    logger.info('The parameter fields in method zcaps.find is being ' +
      'deprecated please use options.projection instead');
  }
  options.projection = options.projection || fields;
