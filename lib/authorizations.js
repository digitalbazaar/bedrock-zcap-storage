...
const COLLECTION_NAME = 'zcap-storage-authorization';

bedrock.events.on('bedrock-mongodb.ready', async () => {
  /* Note: The `zcap-storage-authorization` collection is used to store
  capabilities that are actively authorized for use. The authorizing party
  (stored as the `controller`) writes these capabilities to this collection
  after an application ensures they are permitted to do so for a given
...
export async function insert({controller, capability} = {}) {
  ...
  // insert the capability and return the updated record
  const now = Date.now();
