...
const COLLECTION_NAME = 'zcap-storage-zcap';

bedrock.events.on('bedrock.init', async () => {
  const cfg = bedrock.config['zcap-storage'];
  let cacheConfig = cfg.caches.zcap;
  ...
  ZCAP_CACHE = new LruCache(cacheConfig);
});

bedrock.events.on('bedrock-mongodb.ready', async () => {
  /* Note: The `zcap-storage-zcap` collection is used to store capabilities for
  its controller to later invoke.
